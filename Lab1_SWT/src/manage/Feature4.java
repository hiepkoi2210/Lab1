...
public class Feature4 {
    ...
    public boolean updateUserT(List<User> list, String code, String name, String Password){
        for (int i = 0; i < list.size(); i++) {
            User u = list.get(i);
    ...
    public int addUserT(List<User> list, String code, String name, String password){
        list.add(new User(code, name, password, 1));
        System.out.println("Add successfully");
        return list.size();
    }
    
    public boolean removeUserT(List<User> list, String code){
        for (int i = 0; i < list.size(); i++) {
            User u = list.get(i);
            if(u.getCode().equalsIgnoreCase(code)){
                list.remove(u);
